...
export async function fetchAiSourceData(supabase, userId, range) {
  ...
  if (tasksResponse.error && shouldFallbackTaskQuery(tasksResponse.error)) {
    tasksResponse = await supabase
      .from("tasks")
      .select("id, title, details, status, planned_date, task_type, cognitive_load, emotional_load, is_focus, completed_at, archived_at")
      .eq("user_id", userId)
      .gte("planned_date", range.periodStart)
      .lte("planned_date", range.periodEnd)
      .is("archived_at", null)
...
function shouldFallbackTaskQuery(error) {
